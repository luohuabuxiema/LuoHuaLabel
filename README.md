...
**LuoHuaLabel** 是一款现代化、高效的计算机视觉数据标注工具。系统基于 PySide6 构建，深度集成了最新一代的 **Segment Anything Model 3 (SAM3)**，实现了“点击即标注”和“提示词批量提取”的极速体验。不仅支持矩形（HBB）、多边形（Polygon）、点（Point）标注、和 OBB 旋转框标注。
注意：点标注有待优化，这个功能待测试
...
为了启用 AI 智能标注，你需要下载 SAM3 模型权重文件（如 `sam3.pt`）。
...
**"学习新思想，争做新青年" —— 让我们一起为开源计算机视觉社区添砖加瓦！**
